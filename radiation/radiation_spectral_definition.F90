...
contains
  ...
  subroutine calc_mapping(this, temperature, wavenumber, mapping, use_bands)
    ...
    if (use_bands_local) then
      ...
      ! do jwav = 1, nwav
      !   planck_weight(jwav) = calc_planck_function_wavenumber(wavenumber(jwav), &
      !      &                                          temperature)
      ! end do
      ...
      planck_weight = calc_planck_function_wavenumber(0.5_jprb &
           ...
           &  temperature)
      ! do jwav = 1, size(planck_weight)
      !     planck_weight(jwav) = calc_planck_function_wavenumber(0.5_jprb &
      !      &  * (this%wavenumber1(jwav) + this%wavenumber2(jwav)), &
      !      &  temperature)   
      ! end do
      mapping = 0.0_jprb
...
!                 &       / (wavenum2-wavenum1)
  ...
  ! end function calc_planck_function_wavenumber
   ...
   elemental function calc_planck_function_wavenumber(wavenumber, temperature)
    ...
    freq = wavenumber *  SpeedOfLight

    ! print *, "wavenum", wavenumber, "freq", freq,  "DIVISOR:", &
      ! (exp(PlanckConstant*100*freq/(BoltzmannConstant*temperature)) - 1.0_jprb)

    calc_planck_function_wavenumber = (2.0_jprb * 1e6_jprb * freq * (PlanckConstant * freq**2) * SpeedOfLight**-2) &
         &  / (exp(PlanckConstant*100*freq/(BoltzmannConstant*temperature)) - 1.0_jprb)
