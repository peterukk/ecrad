...
contains
  ...
  subroutine read_input_blocked(file, config, driver_config, nblocks, ncol, nlev, &
    ...
    type(cloud_type),           dimension(:), allocatable,  intent(inout) :: cloud
    ...
    real(jprb), allocatable, dimension(:,:) :: prop_2d, prop_2d_2, prop_2d_3,prop_2d_4,prop_2d_5
    real(jprb), allocatable, dimension(:,:,:)   :: prop_3d

    integer, allocatable, dimension(:) ::  istartcols, iendcols
    integer :: jgas               ! Loop index for reading gases
    integer :: irank              ! Dimensions of gas data
    integer :: blocksize, b, istartcol, iendcol
    ...
    ! Pressure and temperature (SI units) are on half-levels, i.e. of
    ! length (ncol,nlev+1)
    call file%get('pressure_hl',   prop_2d)
    call file%get('temperature_hl',prop_2d_2)

    ! Extract array dimensions
    ncol = size(prop_2d,1)
    nlev = size(prop_2d,2)-1
    ...
    do b = 1, nblocks
      ...
      thermodynamics(b)%pressure_hl     = prop_2d(istartcol:iendcol,:)
      thermodynamics(b)%temperature_hl  = prop_2d_2(istartcol:iendcol,:)
    ...
    if (config%do_clouds) then
      ...
      ! call file%get('re_liquid',     cloud%re_liq)  ! m
      ! call file%get('re_ice',        cloud%re_ice)  ! m
      call file%get('q_liquid',      prop_2d, do_transp=.false.)
      call file%get('q_ice',         prop_2d_2, do_transp=.false.)   ! kg/kg
      call file%get('cloud_fraction',prop_2d_3, do_transp=.false.)
      call file%get('re_liquid',     prop_2d_4, do_transp=.false.)  ! m
      call file%get('re_ice',        prop_2d_5, do_transp=.false.)  ! m
      do b = 1, nblocks
        cloud(b)%q_liq    = transpose(prop_2d(:,istartcols(b):iendcols(b)))
        cloud(b)%q_ice    = transpose(prop_2d_2(:,istartcols(b):iendcols(b)))
        cloud(b)%fraction = transpose(prop_2d_3(:,istartcols(b):iendcols(b)))
        cloud(b)%re_liq   = transpose(prop_2d_4(:,istartcols(b):iendcols(b)))
        cloud(b)%re_ice   = transpose(prop_2d_5(:,istartcols(b):iendcols(b)))
      end do
      ...
      deallocate(prop_2d, prop_2d_2, prop_2d_3, prop_2d_4, prop_2d_5)
      ...
      if (file%exists('iseed')) then
        call file%get('iseed', prop_1d)              ! read iseed as real, 
        do b = 1, nblocks
          single_level(b)%iseed = int(prop_1d(istartcols(b):iendcols(b))) ! convert to integer
        end do
      end if
      ...
      if (driver_config%sw_albedo_override >= 0.0_jprb) then
        ...
        if (file%get_rank('sw_albedo') == 1) then
          ...
          do b = 1, nblocks
            ...
            single_level(b)%sw_albedo_direct  = transpose(prop_2d_2(:,istartcols(b):iendcols(b)))
          end do
          deallocate(prop_2d)
    ...
    do jgas = 1,NMaxGases
      if (jgas == IH2O) then
        if (file%exists('q')) then
          call file%get('q', gas_mr)
          do b = 1,nblocks
            call gas(b)%put(IH2O, IMassMixingRatio, gas_mr(istartcols(b):iendcols(b),:) )
          end do
        else if (file%exists('h2o_mmr')) then
          call file%get('h2o_mmr', gas_mr)
          do b = 1,nblocks
          ...
          call file%get('h2o' // trim(driver_config%vmr_suffix_str), gas_mr)
          do b = 1,nblocks
            call gas(b)%put(IH2O, IVolumeMixingRatio, gas_mr(istartcols(b):iendcols(b),:))
          end do
        end if
      else if (jgas == IO3) then
        if (file%exists('o3_mmr')) then
          call file%get('o3_mmr', gas_mr)
          do b = 1,nblocks
            call gas(b)%put(IO3, IMassMixingRatio, gas_mr(istartcols(b):iendcols(b),:))
          end do
        else
          call file%get('o3' // trim(driver_config%vmr_suffix_str), gas_mr)
          do b = 1,nblocks
            call gas(b)%put(IO3, IVolumeMixingRatio, gas_mr(istartcols(b):iendcols(b),:))
          end do
        end if
      else
        ! Find number of dimensions of the variable holding gas "jgas" in
        ...
        if (irank == 0) then
          ...
          call file%get(trim(gas_var_name), gas_mr)
          do b = 1,nblocks
            call gas(b)%put(jgas, IVolumeMixingRatio, gas_mr(istartcols(b):iendcols(b),:))
          end do
        else if (irank > 0) then
          write(nulout,'(a,a,a)')  '***  Error: ', trim(gas_var_name), ' does not have 0 or 2 dimensions'
          stop
